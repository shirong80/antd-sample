...
export class BaseModalContainerComponent extends BasePortalOutlet implements OnDestroy {
  ...
  get maskClosable(): boolean {
    ...
    return !!getValueWithConfig<boolean>(
      this.config.nzMaskClosable,
      defaultConfig.nzMaskClosable,
      true,
    );
  }
    ...
    if (this.document) {
      ...
      if (this.host.nativeElement.focus) {
        this.ngZone.runOutsideAngular(() =>
          reqAnimFrame(() => this.host.nativeElement.focus()),
        );
      }
    }
  }
  ...
  bindBackdropStyle(): void {
    ...
    if (backdropElement) {
      ...
      if (
        typeof this.config.nzMaskStyle === 'object' &&
        Object.keys(this.config.nzMaskStyle).length
      ) {
