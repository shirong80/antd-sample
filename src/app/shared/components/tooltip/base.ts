...
export abstract class NzTooltipBaseDirective implements OnChanges, OnDestroy, AfterViewInit {
  ...
  protected get _visible(): boolean {
    return (
      (typeof this.visible !== 'undefined' ? this.visible : this.internalVisible) || false
    );
  }
  ...
  protected createComponent(): void {
    ...
    // ? cdkOverlay 에서 컨테이너 포지션을 잘못 잡는 문제 발생.
    // ? 해당 이슈를 반영하여, arrow 배치가 끝난후 overlay 포지션을 다시 계산해야 한다.
    ngVisibleChange$
      .pipe(
    ...
    if (trigger === 'hover') {
      ...
      this.triggerDisposables.push(
        this.renderer.listen(el, 'mouseleave', () => {
          ...
          // * 2. origin 에서 벗어나서 overlay 에 진입할경우
      ...
      // ! 트리거가 focus 인 경우는 사용안함.
      this.triggerDisposables.push(this.renderer.listen(el, 'focusin', () => this.show()));
      this.triggerDisposables.push(this.renderer.listen(el, 'focusout', () => this.hide()));
    } else if (trigger === 'click') {
      // ! 트리거가 click 인 경우는 사용안함.(overlay html 확인용으로 개발단계에서 사용할수 있음.)
      this.triggerDisposables.push(
  ...
  private updatePropertiesByKeys(keys?: string[]): void {
    ...
    (
      keys || Object.keys(mappingProperties).filter((key) => !key.startsWith('directive'))
    ).forEach((property: NzSafeAny) => {
  ...
  private delayEnterLeave(isOrigin: boolean, isEnter: boolean, delay: number = -1): void {
    // * delayTimer 이미 작동하고 있을경우, 해당 타이머를 멈춘다.
    if (this.delayTimer) {
      // * 타이머를 멈춰야 하는 이유
      // * 커서가 origin 에서 벗어난후 overlay 에 이동할 경우,
      // * 커서가 순간적으로 orign 을 스쳐지나는 경우 등등 경우
      // * 기존에 대기중이던 타이머는 더이상 작동하면 안된다.
      this.clearTogglingTimer();
    } else if (delay > 0) {
